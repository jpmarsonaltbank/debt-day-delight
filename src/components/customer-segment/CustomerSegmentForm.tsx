...
const CustomerSegmentForm = ({ initialData, onSubmit, onCancel, existingPriorities = [] }: CustomerSegmentFormProps) => {
  ...
  const handleSubmit = (data: CustomerSegment) => {
    try {
      ...
      const isDuplicatePriority = existingPriorities.includes(priority) && 
        (!initialData || initialData.priority !== priority);
      
      if (isDuplicatePriority) {
        toast({
          title: "Duplicate priority",
          description: "A segment with this priority already exists. Please use a different priority number.",
          variant: "destructive",
        });
        return;
      }
